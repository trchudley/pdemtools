...
def icesat2_atl06(
        ...
        min_is2_points: Optional[int] = 50,
    ...
    if isinstance(target_rxd, (list, tuple)) and len(target_rxd)==4:
        if epsg is None:
            raise ValueError('If querying with a tuple of bounds, `epsg` must be defined.')
        bounds = target_rxd
    ...
    gdf_4326 = gpd.GeoDataFrame(geometry=[box(*bounds)], crs=epsg).to_crs(4326)#.geometry.values[0]
    ...
    for r in days_r:
        ...
        if n_points == 0:
            ...
            print(f"{n_points} points found with {n_points_flt} intersecting dataset. Below minimum threshold ({min_is2_points}).", end=' ')
        else:
    ...
    if n_points < min_is2_points:
        warnings.warn('Returning GeoDataFrame with number of points below minimum threshold.')
        return gdf
    else:
        return gdf
